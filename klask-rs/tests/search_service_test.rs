...
mod search_service_tests {
    ...
    use tokio_test;
    use klask_rs::services::search::{SearchService, SearchQuery, SearchResult};
    ...
    async fn test_search_functionality() {
        ...
        let files = vec![
            (file_ids[0], "main.rs", "src/main.rs", "fn main() { println!(\"Hello, world!\"); }", "rust"),
            (file_ids[1], "lib.rs", "src/lib.rs", "pub fn hello() { println!(\"Hello from lib!\"); }", "rust"),
            (file_ids[2], "test.py", "tests/test.py", "def test_hello(): print(\"Hello, Python!\")", "python"),
        ];
        
        for (id, name, path, content, ext) in files {
            service.upsert_file(id, name, path, content, "test-project", "1.0.0", ext).await.unwrap();
        }
    ...
    async fn test_search_with_filters() {
        ...
        let files = vec![
            (file_ids[0], "main.rs", "src/main.rs", "fn main() { println!(\"Test\"); }", "project-a", "1.0.0", "rs"),
            (file_ids[1], "lib.rs", "src/lib.rs", "pub fn test() { println!(\"Test\"); }", "project-a", "1.1.0", "rs"),
            (file_ids[2], "app.js", "src/app.js", "console.log('Test');", "project-b", "1.0.0", "js"),
            (file_ids[3], "util.py", "src/util.py", "print('Test')", "project-b", "2.0.0", "py"),
        ];
        
        for (id, name, path, content, project, version, ext) in files {
            service.upsert_file(id, name, path, content, project, version, ext).await.unwrap();
        }
    ...
    async fn test_search_with_special_characters() {
        ...
        for query_text in test_queries {
            ...
            assert!(!results.results.is_empty(), "Should have at least one result for: {}", query_text);
        }
    }
    ...
    async fn test_duplicate_prevention_via_upsert() {
        ...
        let matching_results: Vec<&SearchResult> = results.results.iter()
