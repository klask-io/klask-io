...
impl TestSetup {
    ...
    async fn cleanup(&self) -> Result<()> {
        ...
        self.crawler_service.cancel_crawl(self.repository_id).await?;
        self.progress_tracker.remove_progress(self.repository_id).await;
...
async fn test_prevent_concurrent_crawl_same_repository() -> Result<()> {
    ...
    setup.progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
    setup.progress_tracker.update_status(setup.repository_id, CrawlStatus::Processing).await;
    ...
    assert!(response_text.to_lowercase().contains("already") || 
...
async fn test_allow_crawl_after_completion() -> Result<()> {
    ...
    setup.progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
    setup.progress_tracker.complete_crawl(setup.repository_id).await;
    
    // Should not be considered as crawling anymore
    assert!(!setup.progress_tracker.is_crawling(setup.repository_id).await);
...
async fn test_allow_crawl_after_failure() -> Result<()> {
    ...
    setup.progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
    setup.progress_tracker.set_error(setup.repository_id, "Test error".to_string()).await;
...
async fn test_allow_crawl_after_cancellation() -> Result<()> {
    ...
    setup.progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
    setup.progress_tracker.cancel_crawl(setup.repository_id).await;
...
async fn test_crawl_status_check_accuracy() -> Result<()> {
    ...
    // Update to indexing
    setup.progress_tracker.update_status(setup.repository_id, CrawlStatus::Indexing).await;
    assert!(setup.progress_tracker.is_crawling(setup.repository_id).await);
...
async fn test_conflict_prevention_with_different_states() -> Result<()> {
    ...
    for state in test_states {
        ...
        setup.progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
        setup.progress_tracker.update_status(setup.repository_id, state).await;
        ...
        assert_eq!(response.status_code(), StatusCode::CONFLICT, 
                  "Should reject crawl when repository is in state: {:?}", state);
        
        // Clean up for next iteration
        setup.progress_tracker.remove_progress(setup.repository_id).await;
    }
...
async fn test_crawler_service_direct_conflict_prevention() -> Result<()> {
    ...
    setup.crawler_service.cancel_crawl(setup.repository_id).await?;
