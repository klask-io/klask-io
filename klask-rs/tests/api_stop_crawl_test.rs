...
use klask_rs::{
    ...
    services::{SearchService, crawler::CrawlerService, progress::ProgressTracker},
    AppState,
};
...
async fn test_stop_crawl_successful() -> Result<()> {
    ...
    progress_tracker.start_crawl(setup.repository_id, "test-repo".to_string()).await;
...
async fn test_stop_crawl_endpoint_path_variations() -> Result<()> {
    ...
    let response = setup.server
        .delete(&format!("/api/repositories/{}/crawl/extra", setup.repository_id))
        .add_header("Authorization", format!("Bearer {}", setup.admin_token))
        .await;
