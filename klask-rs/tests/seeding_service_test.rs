...
async fn test_seed_all_creates_data() -> Result<()> {
    ...
    assert!(stats.repositories > 0, "Expected repositories to be created");
    ...
    assert_eq!(stats.files, 50, "Expected 50 seed files (10 per repository)");
...
async fn test_seed_users_creates_expected_users() -> Result<()> {
    ...
    let admin_user = sqlx::query("SELECT username, email, role::TEXT as role, active FROM users WHERE username = 'admin'")
        .fetch_one(&pool)
        .await?;
...
async fn test_seed_repositories_creates_expected_repos() -> Result<()> {
    ...
    assert_eq!(klask_react_repo.get::<String, _>("url"), "https://github.com/klask-io/klask-react");
    ...
    let disabled_repo = sqlx::query(
        "SELECT name, enabled FROM repositories WHERE name = 'legacy-system'"
    )
    .fetch_one(&pool)
    .await?;
...
async fn test_seed_files_creates_expected_files() -> Result<()> {
    ...
    let rust_files = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM files WHERE extension = 'rust'"
    )
    .fetch_one(&pool)
    .await?;
    
    let json_files = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM files WHERE extension = 'json'"
    )
    .fetch_one(&pool)
    .await?;
    
    let md_files = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM files WHERE extension = 'md'"
    )
    .fetch_one(&pool)
    .await?;
    ...
    let file_with_content = sqlx::query(
        "SELECT path, content, size FROM files WHERE path = 'src/main.rs' LIMIT 1"
    )
    .fetch_one(&pool)
    .await?;
...
async fn test_concurrent_seeding_operations() -> Result<()> {
    ...
    let (result1, result2) = tokio::join!(
        async move { service1.get_stats().await },
        async move { service2.get_stats().await }
    );
