...
mod crawl_edge_cases_tests {
    ...
    async fn test_concurrent_progress_updates_during_status_changes() {
        ...
        for i in 0..5 {
            let tracker_clone = Arc::clone(&tracker);
            let handle = tokio::spawn(async move {
                tracker_clone.set_current_file(
                    repo_id, 
            ...
            for i in 0..1000 {
                ...
                let handle = tokio::spawn(async move {
                    ...
                    match i % 6 {
                        ...
                        1 => { tracker_clone.update_progress(repo_id, i, Some(1000), i/2).await; },
                        2 => { tracker_clone.set_current_file(repo_id, Some(format!("file-{}", i))).await; },
                        3 => { tracker_clone.get_progress(repo_id).await; },
                        4 => { tracker_clone.get_all_active_progress().await; },
                        _ => { tracker_clone.update_status(repo_id, CrawlStatus::Processing).await; },
                    }
                });
                handles.push(handle);
            }
        ...
        for _ in 0..500 {
            ...
            let handle = tokio::spawn(async move {
                match fastrand::usize(..8) {
                    0 => tracker_clone.is_crawling(repo_id).await,
                    1 => { tracker_clone.update_status(repo_id, CrawlStatus::Processing).await; false },
                    2 => { tracker_clone.update_progress(repo_id, 50, Some(100), 25).await; false },
                    3 => { tracker_clone.set_current_file(repo_id, Some("test.rs".to_string())).await; false },
                    4 => { tracker_clone.complete_crawl(repo_id).await; false },
                    5 => { tracker_clone.cancel_crawl(repo_id).await; false },
                    6 => { tracker_clone.set_error(repo_id, "Random error".to_string()).await; false },
    ...
    async fn test_error_handling_during_concurrent_state_changes() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                tracker_clone.update_progress(repo_id, i * 20, Some(100), i * 10).await;
            });
            handles.push(handle);
        }
