...
impl SchedulerService {
    ...
    pub async fn schedule_repository(&self, repository: &Repository) -> Result<()> {
            ...
            Box::pin(async move {
                ...
                let crawl_result = tokio::time::timeout(crawl_timeout, async {
                    ...
                    if let Err(e) = update_next_crawl_time(&pool, repository_id).await {
                        error!("Failed to update next_crawl_at for repository {}: {}", repository_id, e);
                    }
                    ...
                    match &result {
                        Ok(_) => {
                            info!("Scheduled crawl completed successfully for repository: {} ({})", repository_name, repository_id);
                        },
                        Err(e) => {
                            error!("Scheduled crawl failed for repository {} ({}): {}", repository_name, repository_id, e);
                        }
                    }
                ...
                match crawl_result {
                    Ok(Ok(_)) => {
                        info!("Scheduled crawl for repository {} completed within timeout", repository_id);
                    },
                    Ok(Err(e)) => {
                        error!("Scheduled crawl for repository {} failed: {}", repository_id, e);
                    },
                    Err(_) => {
                        error!("Scheduled crawl for repository {} timed out after {} minutes", repository_id, max_duration);
                    }
                }
            })
        })?;
        ...
        info!("Scheduled repository {} ({}) with cron expression: {}", repository.name, repository_id, schedule_expr);
        Ok(())
    }
    ...
    pub async fn unschedule_repository(&self, repository_id: Uuid) -> Result<()> {
        ...
        if let Some(job_id) = job_ids.remove(&repository_id) {
            if let Err(e) = self.scheduler.remove(&job_id).await {
                error!("Failed to remove scheduled job for repository {}: {}", repository_id, e);
                return Err(e.into());
            }
    ...
    pub async fn reload_scheduled_repositories(&self) -> Result<()> {
                    ...
                    Ok(_) => {
                        ...
                        if repository.next_crawl_at.is_none() {
                            if let Err(e) = update_next_crawl_time(&self.pool, repository.id).await {
                                warn!("Failed to update next_crawl_at for repository {}: {}", repository.id, e);
                            }
                        }
                    },
                    Err(e) => {
                        error!("Failed to schedule repository {} ({}): {}", repository.name, repository.id, e);
                        error_count += 1;
                    }
                }
            }
        ...
        info!("Reloaded {} scheduled repositories ({} errors)", scheduled_count, error_count);
        Ok(())
    }
    ...
    pub async fn get_status(&self) -> Result<SchedulerStatus> {
        ...
        let auto_crawl_enabled_count = all_repositories.iter().filter(|r| r.auto_crawl_enabled).count();
            ...
            Box::pin(async move {
                ...
                if let Ok(repositories) = repository_repo.find_all().await {
                    ...
                    if changes_detected > 0 {
                        debug!("Detected {} repositories with auto-crawl enabled", changes_detected);
                    }
                }
            })
        })?;
...
async fn update_next_crawl_time(pool: &PgPool, repository_id: Uuid) -> Result<()> {
    ...
    let repository = repository_repo.find_by_id(repository_id).await?
    ...
    if let Some(next_time) = next_crawl_at {
        ...
        debug!("Updated next_crawl_at for repository {} to {}", repository_id, next_time);
    }
